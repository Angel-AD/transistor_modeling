...
def _run_experiment_impl(task):
    (master_dir, base_name, base_args, alpha_scale, combiner, lr,
     ...
     gm_vds_min, gm_vgs_min, vgs_thr, knee_vgs_tau, knee_max_correction, add_zero_vds, ids_region,
     knee_lr_scale, region, vds_loss, adamw_avoid_localmin, ids_out_margin,
     vdsgate_output_activation, index) = task
    ...
    # region_weight is a swept dimension but is otherwise absent from the dir name, so two
    # runs that differ ONLY in region_weight (e.g. a [2,4] sweep run into a master root that
    # already holds 0/1 runs) would map to the same exp_NNN dir -- the second then hits the
    # "already finished" skip-guard below and never runs. Encode it (_rw<weight>) so every
    # region_weight gets its own dir and weights {0,1,2,4} can coexist in one master root.
    region_wt = region[4]
    exp_dir = os.path.join(master_dir, f"exp_{index:03d}_{gm_mode}_W{gm1_w}-{gm2_w}-{gm3_w}_s{seed}_rw{region_wt:g}")
    ...
    if ids_out_margin and ids_out_margin > 0.0:
        cmd.extend(["--ids_out_margin", str(ids_out_margin)])
    # vdsgate gate mode (softplus=legacy default -> flag omitted, reproduces exact legacy
    # commands; tanhm -> explicit --vdsgate_output_activation). Only meaningful for
    # vdsgate*/vdsgate_aeff*/vdsgate_vdsk* wrappers; harmlessly ignored otherwise.
    if vdsgate_output_activation and vdsgate_output_activation != "softplus":
        cmd.extend(["--vdsgate_output_activation", str(vdsgate_output_activation)])
    if adamw_avoid_localmin:
        cmd.append("--adamw_avoid_localmin")
...
def build_tasks(exp_name, cfg, defaults, master_root, opt_params_path, csv_path, args):
    ...
    # opt-in AdamW-phase upgrade (cosine warm restarts + grad clipping); off -> legacy flat LR,
    # exactly reproducing every existing run/config unless this is explicitly set to true.
    adamw_avoid_localmin = bool(merged.get("adamw_avoid_localmin", False))
    ...
    # Output-scale margin for bounded output activations (sigmoid/tanh); see --ids_out_margin
    # help in per_neuron_simple_angelov_nn_test.py. SWEPT dimension, same fallback pattern as
    # vds_loss. 0 (default) = off, reproduces exact legacy behavior.
    ids_out_margins = merged.get("ids_out_margins", [merged.get("ids_out_margin", 0.0)])
    # vdsgate gate mode (softplus=legacy default, tanhm=raw-NN gate); see --vdsgate_output_activation
    # help in per_neuron_simple_angelov_nn_test.py. SWEPT dimension, same fallback pattern as
    # ids_out_margin. "softplus" (default) reproduces exact legacy behavior (flag omitted in cmd).
    vdsgate_output_activations = merged.get("vdsgate_output_activations",
                                             [merged.get("vdsgate_output_activation", "softplus")])
    sweep = list(itertools.product(
        ...
        region_weights, vds_losses, ids_out_margins, vdsgate_output_activations,
    ))
    ...
    for variant in variants:
        ...
        for base_name, base_args in merged["base_configs"].items():
            ...
            for (alpha, combiner, lr, arch, out_act, w1, w2, w3, mode, seed, gmr, vgs_thr, vds_min, vgs_min, ids_rw, region_wt, vds_loss, ids_out_margin, vdsgate_out_act) in sweep:
                ...
                tasks.append((master_dir, eff_base_name, eff_base_args, alpha, combiner, lr, arch, out_act,
                              ...
                              lbfgs_epochs, lbfgs_max_iter, lbfgs_gm_aware, loss_norm,
                              gm_warmup_epochs, gm_warmup_lr, ids_constraint, ids_target, ids_lambda,
                              vds_min, vgs_min, vgs_thr, knee_vgs_tau, knee_max_correction, add_zero_vds, ids_region,
                              knee_lr_scale, region, vds_loss, adamw_avoid_localmin, ids_out_margin,
                              vdsgate_out_act, idx))
                idx += 1
...
if __name__ == "__main__":
    ...
    for config_path_str in args.config:
        ...
        provenance = config_data.get("_provenance")   # lineage from gen_config_from_rows

        # Copy the inputs into <master_root>/base_files/ so the results folder is
        # self-contained and portable: the config JSON used by the runner, the measurement
        # CSV, and the base ranked CSV (if any). _run_meta.json records both the local copy
        # (relative, preferred) and the original absolute source, so re-running compile needs
        # no retyping and survives the folder being moved.
        try:
            bf = os.path.join(master_root, "base_files")
            os.makedirs(bf, exist_ok=True)

            def _stash(src):
                """Copy src into base_files/; return its path relative to master_root (or None)."""
                if not src or not os.path.isfile(src):
                    return None
                dst = os.path.join(bf, os.path.basename(src))
                try:
                    if os.path.abspath(src) != os.path.abspath(dst):
                        shutil.copy2(src, dst)
                except Exception:
                    return None
                return os.path.relpath(dst, master_root).replace("\\", "/")

            _meta = {}
            _mp = os.path.join(master_root, "_run_meta.json")
            if os.path.isfile(_mp):
                _meta = json.load(open(_mp, encoding="utf-8"))

            csv_abs = os.path.abspath(csv_path)
            _meta["csv"] = _stash(csv_abs) or _meta.get("csv")
            _meta["csv_src"] = csv_abs

            cfg_rel = _stash(os.path.abspath(config_file_path))
            if cfg_rel:
                cfgs = _meta.get("configs") or []
                if cfg_rel not in cfgs:
                    cfgs.append(cfg_rel)
                _meta["configs"] = cfgs

            if provenance:
                base_src = provenance.get("base_csv")
                if base_src:
                    _meta["base_csv"] = _stash(base_src) or _meta.get("base_csv") or base_src
                    _meta["base_csv_src"] = base_src
                _meta["base_exp"] = provenance.get("base_exp")
        ...
        for exp_name, cfg in EXPERIMENTS.items():
            ...
            # Drop the provenance next to the runs so compile can fill base_exp/base_id/base_csv.
            if provenance:
                try:
                    with open(os.path.join(master_dir, "_provenance.json"), "w", encoding="utf-8") as pf:
