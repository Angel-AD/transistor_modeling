...
SCRIPT_PATH = os.path.join(HERE, "per_neuron_simple_angelov_nn_test_simplegate.py")
...
def _run_experiment_impl(task):
    ...
    # SIMPLEGATE: no separate vdsgate gate-mode flag -- vdsgate*/vdsgate_aeff*/vdsgate_vdsk*
    # wrappers use --output_activation directly (already forwarded above via out_act).
...
def build_tasks(exp_name, cfg, defaults, master_root, opt_params_path, csv_path, args):
    ...
    # SIMPLEGATE: no separate vdsgate gate-mode dimension -- output_activations (already in the
    # product below) IS the gate for vdsgate*/vdsgate_aeff*/vdsgate_vdsk* wrappers.
    sweep = list(itertools.product(
        ...
        region_weights, vds_losses, ids_out_margins,
    ))
    ...
    for variant in variants:
        ...
        for base_name, base_args in merged["base_configs"].items():
            ...
            for (alpha, combiner, lr, arch, out_act, w1, w2, w3, mode, seed, gmr, vgs_thr, vds_min, vgs_min, ids_rw, region_wt, vds_loss, ids_out_margin) in sweep:
