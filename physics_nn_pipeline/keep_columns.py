"""
keep_columns.py  --  Write a slimmed xlsx keeping only specified columns, from a .csv or .xlsx source.

Reads the source (header in row 1) and writes a NEW <name><suffix>.xlsx containing only the
--keep columns, in the order given. The source is never modified. Columns in --keep that
aren't present in a file are skipped (with a note), so it's safe across files with slightly
different schemas. .csv sources go straight to a slim .xlsx -- no full .xlsx is ever written.

Usage
-----
    python keep_columns.py --xlsx a.xlsx b.xlsx
    python keep_columns.py --csv a.csv b.csv --keep id eq combined_gm ids_rmse --suffix _slim
"""
...
DEFAULT_KEEP = ["id", "run_id", "arch_id", "arch_hash", "base_id", "eq", "combined_gm", "ids_rmse",
                "region_knee_combined_gm", "region_knee_ids_rmse",
                "region_weight", "vds_loss", "region_vds_hi", "region_vds_lo",
...
def _read_rows(src: Path):
    """[[header...], [row1...], ...] from a .csv or .xlsx source."""
...
def slim(src: Path, keep, suffix: str):
    rows = _read_rows(src)
    if not rows:
...
def main():
    ...
    src = ap.add_argument_group("source (at least one required)")
    src.add_argument("--xlsx", nargs="+", default=[], help="Source xlsx file(s).")
    src.add_argument("--csv", nargs="+", default=[],
                     help="Source csv file(s) -- slimmed directly to .xlsx, no full xlsx written.")
    ...
    if not args.xlsx and not args.csv:
        ap.error("need at least one of --xlsx or --csv.")

    print(f"keep: {args.keep}")
    n = 0
    for x in args.xlsx + args.csv:
        p = Path(x)
        if not p.is_file():
            print(f"  [skip] not found: {p}")
            continue
        if slim(p, args.keep, args.suffix):
            n += 1
