...
def generate_and_write_equation_md(model, X_train_np, calc_metrics,
    ...
    """
    device = device or torch.device('cpu')
    from per_neuron_models import DynamicNN
    ...
    # --- vdsgate / vdsgatelin structured output: Ids = gate(NN) * tanh(alpha*Vds) * (1 + lamb*Vds) ---
    _wrapper = getattr(model, 'nn_wrapper', 'identity')
    _is_vdsgate = _wrapper in ('vdsgate', 'vdsgatelin')
    if _is_vdsgate:
    ...
    if _is_vdsgate:
        _vds_col = X_raw[:, 1]
        _gate_np = np.log1p(np.exp(eq_preds)) if _wrapper == 'vdsgate' else eq_preds
    ...
    def _wrap_out_ads(nn_raw_expr):
        ...
        if not _is_vdsgate:
            return nn_raw_expr
        gate = f"ln(1+exp({nn_raw_expr}))" if _wrapper == 'vdsgate' else f"({nn_raw_expr})"
        return f"({gate}) * tanh(({_vg_alpha:.8f})*_v2) * (1 + ({_vg_lamb:.8f})*_v2)"
    ...
    inner += f" + ({_fw(b_o[0])})"
    _out_label = f"{acts_o[0]} -> {_wrapper}(NN)*tanh(a*Vds)*(1+l*Vds)" if _is_vdsgate else acts_o[0]
    ...
    def _wrap_out_va(nn_raw_expr):
        ...
        if not _is_vdsgate:
            return nn_raw_expr
        gate = f"ln(1.0+exp({nn_raw_expr}))" if _wrapper == 'vdsgate' else f"({nn_raw_expr})"
        return f"({gate}) * tanh(({_vg_alpha:.8f})*Vds) * (1.0 + ({_vg_lamb:.8f})*Vds)"
    ...
    inner += f" + ({_fw(b_o[0])})"
    if _is_vdsgate:
        va.append(f"    // Output  ({_wrapper} gate, alpha={_vg_alpha:.6f}, lamb={_vg_lamb:.6f}):  Ids = gate(NN)*tanh(alpha*Vds)*(1+lamb*Vds)")
    else:
...
def main():
    ...
    if _layers_trunc is not None:
        ...
        plot_grid(plot_data, calc_metrics, saved_metrics, eq_metrics,
                  ...
                  list(unique_vds_nominal), list(unique_vgs_nominal),
                  use_real_voltages=False, eq_plot_data=eq_plot_data)
