...
from per_neuron_models import DynamicNN, PhysicsInformedNN, set_mixed_init_mode
...
def main():
    ...
    parser.add_argument("--vdsgate_output_activation", type=str, default="softplus",
                        choices=["softplus", "tanhm"],
                        help="How the vdsgate*/vdsgate_aeff*/vdsgate_vdsk* GATE (the multiplicative "
                             "magnitude term) turns the raw NN output into a value. Orthogonal to "
                             "--equation_type (which family/order). SELF-CONTAINED: both modes apply "
                             "their own squashing directly to the raw NN output, so use "
                             "--output_activation linear for BOTH modes (same as legacy) -- there is "
                             "exactly one squash, done by the gate, not two. Ignored by non-vdsgate "
                             "wrappers and by vdsgatelin (already raw, no gate function). "
                             "'softplus' (default) = softplus(NN) >= 0, unchanged legacy behavior -- "
                             "guarantees sign(Ids)=sign(Vds) but has a vanishing-gradient tail toward "
                             "pinch-off (gate->0 only asymptotically as NN->-inf). 'tanhm' = "
                             "gate = tanh(NN) in (-1,1); combine with --ids_out_margin (rescales to "
                             "the real Ids range) for the full 'tanh_margin' gate: crosses exactly 0 "
                             "(pinch-off) AT NN=0 with tanh's OWN maximum gradient there, instead of "
                             "softplus's vanishing tail. Trade-off: no sign guarantee (gate can go "
                             "negative), same as vdsgatelin.")
    ...
    parser.add_argument("--output_activation", type=str, default="linear", help="Activation function for the output layer")
    ...
    model = PhysicsInformedNN(
        ...
        vdsgate_gate_mode=args.vdsgate_output_activation,
    ).double()
    ...
    run_info = {
        ...
        'output_activation': args.output_activation,
        'vdsgate_output_activation': args.vdsgate_output_activation,   # gate mode for vdsgate*
                                   # wrappers (softplus=legacy default, tanhm=raw-NN gate);
                                   # plot_saved_state.py reads this back to reconstruct the model
