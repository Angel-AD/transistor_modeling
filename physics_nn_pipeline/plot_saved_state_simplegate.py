...
from per_neuron_models_simplegate import DynamicNN, PhysicsInformedNN
...
def _build_model_from_dir(save_dir, device):
    ...
    # SIMPLEGATE: no separate vdsgate gate-mode field -- the gate is whatever
    # output_activation the run used, already reconstructed via base_model above.
...
def _extract_nn_layers(base_nn):
    ...
    from per_neuron_models_simplegate import PerNeuronLinear
    layers = []
...
def generate_and_write_equation_md(model, X_train_np, calc_metrics,
    ...
    """
    device = device or torch.device('cpu')
    from per_neuron_models_simplegate import DynamicNN, aeff_spec, vdsk_spec
    ...
    #   vdsgate / vdsgate_freelam : Ids = gate(NN) * tanh(alpha*Vds) * (1 + lamb*Vds)          [scalar alpha, lamb]
    ...
    # ("vdsgatelin" was DROPPED -- it was a pure alias for "vdsgate" once the gate became
    # output_activation directly; per_neuron_models_simplegate.py errors on it now.)
    _wrapper = getattr(model, 'nn_wrapper', 'identity')
    _is_vdsgate_simple = _wrapper in ('vdsgate', 'vdsgate_freelam')
    ...
    # SIMPLEGATE: no separate gate-mode attribute -- the gate is always nn_raw, i.e. whatever
    # output_activation already produced. Applies to ALL vdsgate*/vdsgate_aeff*/vdsgate_vdsk*
    # families. "vdsgate"'s lamb is forced >=0 via softplus by default (same convention as
    # vdsgate_aeff*/vdsgate_vdsk*); "vdsgate_freelam" leaves it raw/unconstrained.
    if _is_vdsgate_simple:
        _vg_alpha = float(torch.nn.functional.softplus(model.vdsgate_alpha_raw).item())
        _vg_lamb  = (float(model.vdsgate_lamb_raw.item()) if _wrapper == 'vdsgate_freelam'
                     else float(torch.nn.functional.softplus(model.vdsgate_lamb_raw).item()))
    ...
    if _is_vdsgate_simple:
        _vds_col = X_raw[:, 1]
        _gate_np = eq_preds   # SIMPLEGATE: gate = nn_raw (output_activation already applied above)
        ...
        # SIMPLEGATE: gate = nn_raw (output_activation already applied above via _eval_eq_numpy).
        _gate_np = eq_preds
    ...
    def _wrap_out_ads(nn_raw_expr):
        ...
        SIMPLEGATE: gate = nn_raw_expr directly -- it already has output_activation applied
        (see _act_str(acts_o[0], inner) at the call site), no extra squash here."""
        if _is_vdsgate_simple:
            gate = f"({nn_raw_expr})"
    ...
    inner += f" + ({_fw(b_o[0])})"
    if _is_vdsgate_simple:
        _out_label = f"{acts_o[0]}(NN) * tanh(a*Vds)*(1+l*Vds)  [{_wrapper}]"
    elif _is_vdsgate_aeff:
        _out_label = f"{acts_o[0]}(NN) * tanh(a_eff(Vgs)*Vds)*(1+l_eff(Vgs)*Vds)  [{_wrapper}]"
    elif _is_vdsgate_vdsk:
        _out_label = f"{acts_o[0]}(NN) * tanh(Vds/Vds_knee(Vgs))*(1+l_eff(Vgs)*Vds)  [{_wrapper}]"
    else:
        _out_label = acts_o[0]
    md.append(f"## Output  (activation = {_out_label})\n")
    md.append(f"    Ids [A] = {_wrap_out_ads(_act_str(acts_o[0], inner))}\n")
    if _is_vdsgate_simple:
        _lamb_note = "raw, unconstrained" if _wrapper == 'vdsgate_freelam' else ">=0, softplus"
        md.append(f"    [vdsgate params: alpha={_vg_alpha:.6f} (>0, softplus), lamb={_vg_lamb:.6f} ({_lamb_note})]\n")
    elif _is_vdsgate_aeff or _is_vdsgate_vdsk:
    ...
    def _wrap_out_va(nn_raw_expr):
        ...
        and the ids_scale multiplier (--ids_out_margin) if this run used one.
        SIMPLEGATE: gate = nn_raw_expr directly -- it already has output_activation applied
        (see _act_str_va(acts_o[0], inner) at the call site), no extra squash here."""
        if _is_vdsgate_simple:
            gate = f"({nn_raw_expr})"
