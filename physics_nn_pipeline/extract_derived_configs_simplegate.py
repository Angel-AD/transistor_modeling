...
def _layer_activations(arch_str: str) -> set[str] | None:
    """Same definition used by find_consistent_archs.py: every activation name used anywhere
    in an architecture string, or None if unparseable."""
    import json as _json
    try:
        layers = _json.loads(arch_str)
...
def _tanh_only_hashes(base_csv: Path) -> set[str]:
    """arch_hash set for every architecture in base_csv whose every layer is 'tanh' only."""
    hashes = set()
    for r in _read_rows(base_csv):
        h = r.get("arch_hash")
        arch_str = r.get("architecture", "")
        if h and arch_str and _layer_activations(arch_str) == {"tanh"}:
            hashes.add(h)
    return hashes
...
def do_bothshapeok(folder: str, folder_root: Path, out_configs_dir: Path, python_exe: str, dry_run: bool,
                    top_n: int | None = None, gm_ceiling_start: float = 0.9, tanh_only: bool = False,
                    output_folder: str | None = None):
    output_folder = output_folder or folder
    base_csv = _base_ranked_csv(folder_root)
    if base_csv is None:
        print(f"  SKIP {output_folder}: no base ranked csv found under {folder_root}")
        return
    ...
    if dry_run:
        ...
        tanh_note = " [TANH-ONLY]" if tanh_only else ""
        print(f"  (dry run) would compute bothshape_ok ids from {shape_csv.name}{cap_note}{tanh_note} and gen_config")
        return
    ...
    print(f"  {output_folder}: {len(ok_rows)} / {len(rows)} bothshape_ok")
    if not ok_rows:
        print(f"  SKIP gen_config for {output_folder}: 0 bothshape_ok rows")
        return

    if top_n is not None and len(ok_rows) > top_n:
        ok_rows, gm_ceiling = _escalate_and_select(ok_rows, top_n, gm_ceiling_start)
        print(f"  {output_folder}: capped to best {len(ok_rows)} bothshape_ok rows "
    ...
    out_path = out_configs_dir / f"{output_folder}_bothshapeok.json"
    _run([python_exe, str(GEN_CONFIG),
          "--csv", str(base_csv),
          "--ids", ",".join(ok_ids),
          "--out", str(out_path),
          "--prefix", f"{output_folder}_bothshapeok"], dry_run)


def do_best100_gmshapeok(folder: str, folder_root: Path, out_configs_dir: Path, python_exe: str,
                          top_n: int, gm_ceiling_start: float, dry_run: bool, tanh_only: bool = False,
                          output_folder: str | None = None):
    output_folder = output_folder or folder
    base_csv = _base_ranked_csv(folder_root)
    if base_csv is None:
    ...
    if dry_run:
        tanh_note = " [TANH-ONLY]" if tanh_only else ""
        print(f"  (dry run) would select top {top_n} gmshape_ok rows from {shape_csv.name}{tanh_note} and gen_config")
        return
    ...
    print(f"  {output_folder}: {len(gm_ok_rows)} / {len(rows)} gmshape_ok")
    if not gm_ok_rows:
        print(f"  SKIP gen_config for {output_folder}: 0 gmshape_ok rows")
        return

    chosen, gm_ceiling = _escalate_and_select(gm_ok_rows, top_n, gm_ceiling_start)
    print(f"  {output_folder}: gmshape_ok & combined_gm<={gm_ceiling} gives best {len(chosen)} by ids_rmse (need >={top_n})")
    chosen_ids = [r["id"] for r in chosen]

    out_configs_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_configs_dir / f"{output_folder}_best100_gmshapeok.json"
    _run([python_exe, str(GEN_CONFIG),
          "--csv", str(base_csv),
          "--ids", ",".join(chosen_ids),
          "--out", str(out_path),
          "--prefix", f"{output_folder}_best100_gmshapeok"], dry_run)
...
def do_best200(folder: str, folder_root: Path, out_configs_dir: Path, python_exe: str,
                ...
                tanh_only: bool = False, output_folder: str | None = None):
    output_folder = output_folder or folder
    base_csv = _base_ranked_csv(folder_root)
    if base_csv is None:
        print(f"  SKIP {output_folder}: no base ranked csv found under {folder_root}")
    ...
    if tanh_only:
        ...
        tanh_hashes = _tanh_only_hashes(base_csv)
        rows = [r for r in rows if r.get("arch_hash") in tanh_hashes]
        print(f"  {output_folder}: {len(rows)} tanh-only rows in population")
        if not rows:
            print(f"  SKIP gen_config for {output_folder}: 0 tanh-only rows")
            return
        if dry_run:
            print(f"  (dry run) would gate-then-rank top {top_n} of {len(rows)} tanh-only rows and gen_config")
            return
        chosen, gm_ceiling = _escalate_and_select(rows, top_n, gm_ceiling_start)
        print(f"  {output_folder}: combined_gm<={gm_ceiling} gives best {len(chosen)} tanh-only by ids_rmse "
              f"(need >={top_n}, pool={len(rows)})")
        chosen_ids = [r["id"] for r in chosen]

        out_configs_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_configs_dir / f"{output_folder}_best200.json"
        _run([python_exe, str(GEN_CONFIG),
              "--csv", str(base_csv),
              "--ids", ",".join(chosen_ids),
              "--out", str(out_path),
              "--prefix", f"{output_folder}_best200"], dry_run)
        return
    ...
    while n_pass < top_n and gm_ceiling < 20.0:
        gm_ceiling = round(gm_ceiling * 1.5, 4)
        n_pass = sum(1 for r in rows if (_f(r, "region_knee_combined_gm") or 1e9) <= gm_ceiling)
    print(f"  {output_folder}: combined_gm<={gm_ceiling} gives {n_pass} rows (need >={top_n}), "
    ...
    if rc != 0:
        print(f"  FAILED filter_results.py for {output_folder}")
        return
    ...
    if not filtered:
        print(f"  FAILED: no filter #{filter_number} output found for {output_folder}")
        return
...
def main():
    ...
    output_folder_suffix = "_tanhonly" if args.tanh_only else ""
    for folder in folders:
        folder_root = source_root / folder
        output_folder = f"{folder}{output_folder_suffix}"
        if not folder_root.is_dir():
            print(f"SKIP {output_folder}: {folder_root} does not exist")
            continue
        ...
        if not args.skip_bothshapeok:
            do_bothshapeok(folder, folder_root, bothshapeok_out, args.python_exe, args.dry_run,
                           ...
                           tanh_only=args.tanh_only, output_folder=output_folder)
