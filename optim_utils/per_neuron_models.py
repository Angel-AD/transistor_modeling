...
def aeff_spec(wrapper):
    ...
    (The gate function itself -- softplus(NN) vs a raw-NN 'tanhm' alternative -- is a SEPARATE,
    orthogonal choice: see PhysicsInformedNN's vdsgate_gate_mode / --vdsgate_output_activation,
    not encoded in this wrapper string.)"""
...
class PhysicsInformedNN(nn.Module):
    ...
    def _vdsgate_gate(self, nn_raw):
        """The vdsgate*/vdsgate_aeff*/vdsgate_vdsk* gate function -- see vdsgate_gate_mode in
        __init__ for the two modes. Self-contained: both modes apply their own squashing
        directly to nn_raw, independent of output_activation (use output_activation='linear'
        for BOTH modes, same as legacy -- there is exactly one squash, done here, not two).
        Not used by vdsgatelin (always raw, unaffected by this)."""
        if self.vdsgate_gate_mode == "tanhm":
            return torch.tanh(nn_raw)
        return F.softplus(nn_raw)

    def __init__(self, base_nn, equation_type="product", normalization_config=None, config_key=1,
                 vdsgate_gate_mode="softplus"):
        super().__init__()
        self.base_nn = base_nn
        self.norm_config = normalization_config
        self.config_key = config_key
        # How the vdsgate*/vdsgate_aeff*/vdsgate_vdsk* GATE (the multiplicative magnitude term)
        # turns the raw NN output into a value -- orthogonal to which wrapper/equation family is
        # used, and SELF-CONTAINED: both modes apply their own squashing to nn_raw directly (see
        # _vdsgate_gate above), so output_activation should be "linear" for BOTH modes (same as
        # legacy) -- there is exactly one squash, done by the gate, not two.
        # "softplus" (default, legacy, unchanged behavior) -> softplus(nn_raw) >= 0, guarantees
        # sign(Ids)=sign(Vds) but has a vanishing-gradient tail toward pinch-off (gate->0 only
        # asymptotically as nn_raw->-inf).
        # "tanhm" -> gate = tanh(nn_raw) in (-1,1). Combine with --ids_out_margin (rescales to
        # the real Ids range) for the full "tanh_margin" gate. tanh crosses exactly 0 (gate=0,
        # i.e. pinch-off) AT nn_raw=0 with its OWN maximum gradient there, instead of softplus's
        # vanishing tail. Trade-off: no sign(Ids)=sign(Vds) guarantee (gate can go negative),
        # same as vdsgatelin. Ignored by non-vdsgate wrappers and by vdsgatelin (which already
        # uses the raw NN with no gate function).
        if vdsgate_gate_mode not in ("softplus", "tanhm"):
            raise ValueError(f"vdsgate_gate_mode must be 'softplus' or 'tanhm', got {vdsgate_gate_mode!r}")
        self.vdsgate_gate_mode = vdsgate_gate_mode
        # ---------------------------------------------------------
        ...
        if self.mode in ("noNN", "noNN_knee"):
            ...
            if self.nn_wrapper in ("vdsgate", "vdsgatelin"):
                ...
                # (alpha forced > 0 via softplus in forward). gate sets the magnitude:
                #   vdsgate    -> softplus(NN) >= 0 (guarantees sign(Ids)=sign(Vds))
                #   vdsgatelin -> NN linear (more flexible; no sign guarantee)
            ...
            if not (self.mode == "pure" and (self.nn_wrapper in ("identity", "vdsgate", "vdsgatelin")
    ...
    def forward(self, *args):
        ...
        if self.mode == "noNN":
            ...
            if self.nn_wrapper == "tanh": nn_act = torch.tanh(nn_raw)
            ...
            elif self.nn_wrapper in ("vdsgate", "vdsgatelin"):
                # Ids = gate(NN) * tanh(alpha*Vds) * (1 + lamb*Vds).
                # tanh(alpha*Vds) -> exact 0 at Vds=0 and odd reverse branch; alpha=softplus(raw)>0.
                #   vdsgate    -> gate = self._vdsgate_gate(nn_raw) (softplus by default, see
                #                 vdsgate_gate_mode -- guarantees sign(Ids)=sign(Vds) unless
                #                 vdsgate_gate_mode="tanhm", see that mode's docstring above)
                #   vdsgatelin -> gate = NN linear always (may be negative -> no sign guarantee,
                #                 more flexible; vdsgate_gate_mode has no effect here, it's
                #                 already the "raw" behavior)
                g_alpha = F.softplus(self.vdsgate_alpha_raw)
                g_gate  = self._vdsgate_gate(nn_raw) if self.nn_wrapper == "vdsgate" else nn_raw
                nn_act = g_gate * torch.tanh(g_alpha * phys_vds) * (1.0 + self.vdsgate_lamb_raw * phys_vds)
                ...
                # Gate function (softplus(NN) vs raw NN passthrough) is set by
                # vdsgate_gate_mode / --vdsgate_output_activation, NOT by the wrapper string.
                ...
                nn_act = self._vdsgate_gate(nn_raw) * torch.tanh(a_eff * phys_vds) * (1.0 + l_eff * phys_vds)
                ...
                # Gate function set by vdsgate_gate_mode / --vdsgate_output_activation (same as aeff).
                ...
                l_eff    = poly_l if _free_lam else F.softplus(poly_l)
                nn_act = self._vdsgate_gate(nn_raw) * torch.tanh(phys_vds / Vds_knee) * (1.0 + l_eff * phys_vds)
    ...
    def get_eq_details(cls, equation_type):
        ...
        valid_wrappers = ["tanh", "sigmoid", "softplus", "exp", "invexp", "angelovwrap", "vdsgate", "vdsgatelin", "vdsgate_aeff", "vdsgate_aeff_lin", "vdsgate_aeff_quad", "vdsgate_aeff_cub", "vdsgate_aeff_lin_sig", "vdsgate_aeff_quad_sig", "vdsgate_aeff_cub_sig", "vdsgate_aeff_sig", "vdsgate_vdsk", "vdsgate_vdsk_lin", "vdsgate_vdsk_quad", "vdsgate_vdsk_cub", "identity"]
