...
from the `ids_out_margin` config value (`margin` in the formula above —
the fraction of extra headroom on top of the largest measured current, not
the multiplier itself). With `margin = 0` (the default), `scale = 1` and
bounded activations are left uselessly capped at ±1 — so any project using
